...
- une **application mobile** en **Flutter** (bonus) consommant la même API
- un pipeline **CI/CD** GitHub Actions + déploiement **Docker / Google Cloud Platform** (bonus)
...
├── mobile/           # Application mobile Flutter (bonus)
├── .github/workflows/ci-cd.yml   # Pipeline CI/CD (bonus)
...
| État d'authentification | Context API + localStorage | Simplicité adaptée à la taille du projet (pas besoin de Redux) |
...
Prérequis : **Java 17**, **Maven**, **MySQL** (ou utiliser le profil H2 pour tester sans base).

```bash
cd backend

# Option A — avec MySQL local
# Créez d'abord une base "taskmanager" dans votre MySQL, puis :
...
⚠️ Sur émulateur Android, l'URL `http://10.0.2.2:8080/api` (déjà configurée dans `lib/services/api_service.dart`) pointe vers le `localhost` de votre machine. Sur un appareil physique, remplacez cette URL par l'IP de votre machine ou l'URL publique déployée.
...
- [x] Liste des tâches en temps réel (fetch API via Axios)
- [x] Ajout, édition, suppression de tâches (modale)
- [x] Filtrage par statut + recherche texte (avec debounce)
...
- README avec instructions d'installation et description technique (ce fichier)
- Pipeline CI/CD prêt à l'emploi (bonus)
